...
public class PentahoSamlUserDetailsService implements SAMLUserDetailsService, UserDetailsService {
	...
	public Object loadUserBySAML(SAMLCredential credential) throws UsernameNotFoundException {
		...
		logger.debug(String.format("Loading details for credentials [%s]", new Object[] { credential }));
		UserDetailsService myService = getUserDetailsService();
		logger.debug("UserDetailsService [" + myService.getClass() + "]");
		if (getUserDetailsService() instanceof SAMLUserDetailsService) {
			logger.debug("Loading by means of SAML");
			Object userDetails = ((SAMLUserDetailsService) getUserDetailsService()).loadUserBySAML(credential);
			if (userDetails == null) {
				logger.warn("Got a null from calling the method loadUserBySAML( SAMLCredential credential ) of UserDetailsService: " + getUserDetailsService()
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		try {
			...
			if (user == null) {
				logger.warn("Got a null from calling the method loadUserByUsername( String username ) of UserDetailsService: " + getUserDetailsService()
						+ ". This is an interface violation beacuse it is specified that loadUserByUsername method should never return null. Throwing a UsernameNotFoundException.");
			...
			if (isCreateDetailsOnUsernameNotFoundException()) {
				logger.warn("No user found for Username '" + username + "' in UserDetailsService '" + getSelectedAuthorizationProvider()
						+ "'. Creating an UserDetails with Username '" + username
		...
		return new User(user.getUsername(), user.getPassword(), user.isEnabled(), user.isAccountNonExpired(), user.isCredentialsNonExpired(),
				user.isAccountNonExpired(), newAuthorities);
	}
	...
	private Collection<? extends GrantedAuthority> ensureDefaultRole(Collection<? extends GrantedAuthority> userAuthorities) {
		...
		// This sort is needed because the collection of GrantedAuthorities has to be sorted by natural order, as stated
		// in
